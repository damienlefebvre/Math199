...
namespace stock_namespace {
  ...
  void Stock_c::change_ticker_c() {
    ...
    std::cout << "C++: strncopy \"GOLD\" to this->ticker without going out of bounds" << std::endl;
    strncpy(this->ticker, "GOLD", strlen(this->ticker));
  ...
  const char* Stock_c::get_industry_c() {
...
int main() {
  ...
  char* ticker = (char*)"ABX";

  int price_data_size = 4;
  int historical_price_data_size = 8;
